...
const LiveDiscussion = {
  ...
  async all({ category, search, activeOnly = false, includeReplies = true } = {}) {
    ...
    if (includeReplies && rows.length > 0) {
      const allReplies = await db.prepare(`SELECT * FROM live_discussion_replies ORDER BY created_at ASC, id ASC`).all();
      const replyMap = {};
  ...
  async findById(id) {
    ...
    const flatReplies = await db.prepare(`
      SELECT * FROM live_discussion_replies WHERE discussion_id = ? ORDER BY created_at ASC, id ASC
    `).all(id);
    return {
      ...row,
  ...
  async addReply(discussionId, { userId, authorName, authorRole, content, isMentor = 0, parentId = null }) {
    ...
    if (parentId) {
      const parent = await db.prepare(`SELECT id, discussion_id FROM live_discussion_replies WHERE id = ?`).get(parentId);
